package com.stas.androidrx.fragment;
...
import com.stas.androidrx.Const;
import com.stas.androidrx.R;
import com.stas.androidrx.adapter.ModelsListRecyclerAdapter;
...
public class ModelsListFragment extends Fragment {
    ...
    private void showLoadingIndicator(boolean show) {
        ...
        if (isLoading) {
            if (refreshItem != null) {
                refreshItem.getActionView().animate().setInterpolator(new AccelerateDecelerateInterpolator()).rotationBy(360).setDuration(500).setListener(new AnimatorListenerAdapter() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        refreshItem.getActionView().animate().setInterpolator(new AccelerateDecelerateInterpolator()).rotationBy(360).setDuration(500).setListener(this);
                    }
                });
            }
        } else {
            if (refreshItem != null) {
                refreshItem.getActionView().animate().cancel();
            }
        }
    }
        ...
        subscription = RetrofitSingleton.getModelsObservable().
                ...
                subscribe(new Subscriber<ArrayList<Model>>() {
                    ...
                    public void onNext(ArrayList<Model> newModels) {
                        Log.i(TAG, "onNext: " + newModels.size());
